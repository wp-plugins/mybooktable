...
function mbt_shorcodes_init() {
	...
	if((current_user_can('edit_posts') || current_user_can('edit_pages')) && get_user_option('rich_editing') == 'true') {
		if(in_array(basename($_SERVER['PHP_SELF']), array('post.php', 'page.php', 'page-new.php', 'post-new.php'))) {
			add_filter('media_buttons', 'mbt_shortcode_button', 30);
			add_action('admin_footer', 'mbt_shortcode_form');
		}
	}
}
...
function mbt_get_shortcodes() {
	function get_taxonomy_names($tax) {
		$names = array();
		$terms = get_terms($tax);
	...
	return apply_filters('mbt_shortcodes', $shortcodes);
}
...
function mbt_shortcode_form() {
	...
	function mybooktable_insert_shortcode() {
		...
		var attrs = {};
		jQuery('#mybooktable_shortcode_form_' + shortcode + ' .mybooktable_shortcode_field').each( function(){
	...
	jQuery(function($) {
		...
		$('.shortcode-modal-content .shortcode-menu-item').on( 'click', function() {
			$('.shortcode-modal-content .shortcode-menu-item').removeClass('active');
			$(this).addClass( 'active' );
			$('.mybooktable_shortcode_form_atts').css( 'display', 'none' );
			$('#mybooktable_shortcode_form_' + $(this).data('shortcode') ).css( 'display', 'block' );
		});
				...
				<div class="media-frame-content">
					...
					<div id="mybooktable_shortcode_form_<?php echo $shortcode; ?>" class="mybooktable_shortcode_form_atts" style="display:none">
						<?php if ( !empty($atts['description']) ) { ?>
							<div class="mybooktable_shortcode_description">
							...
							<?php foreach ( $atts['settings'] as $setting => $params ) {
								...
								switch ( $params['type'] ) {
									case 'dropdown':
										...
										if ( ! empty($params['title']) ) echo "<label for='mybooktable_{$shortcode}_field_$setting'>$params[title]</label><br>";
										if ( ! empty($params['description']) ) echo '<div class="description">' . $params['description'] . '</div>';
										if ( ! empty($params['choices']) ) {
											echo "<select class='mybooktable_shortcode_field' id='mybooktable_{$shortcode}_field_$setting' name='$setting' style='max-width:440px;'>";
									...
									case 'thumbsize':
										...
										if ( ! empty($params['title']) ) echo "<label for='mybooktable_{$shortcode}_field_$setting'>$params[title]</label><br>";
										if ( ! empty($params['description']) ) echo '<div class="description">' . $params['description'] . '</div>';
										echo "<select class='mybooktable_shortcode_field' id='mybooktable_{$shortcode}_field_$setting' name='$setting'>";
									...
									case 'checkbox':
										if ( ! empty($params['description']) ) echo '<div class="description">' . $params['description'] . '</div>';
										echo "<input type='checkbox' class='mybooktable_shortcode_field' id='mybooktable_{$shortcode}_field_$setting' name='$setting'>";
										if ( ! empty($params['title']) ) echo " <label for='mybooktable_{$shortcode}_field_$setting'>$params[title]</label>";
										break;
									case 'radio':
										...
										if ( ! empty($params['choices']) ) {
											...
											foreach( $params['choices'] as $key => $value ) {
												echo "<li><input type='radio' class='mybooktable_shortcode_field' id='mybooktable_{$shortcode}_field_{$setting}_{$key}' name='$setting' value='$key'>";
												echo " <label for='mybooktable_{$shortcode}_field_{$setting}_{$key}'>$value</label></li>";
											}
											echo '</ul>';
										}
										break;
									...
									case 'textarea':
										if ( ! empty($params['title']) ) {
											echo "<label for='mybooktable_{$shortcode}_field_$setting'>$params[title]";
											if ( ! empty($params['default']) ) echo " <em>(default: $params[default])</em>";
											echo "</label><br>";
										}
										if ( ! empty($params['description']) ) echo '<div class="description">' . $params['description'] . '</div>';
										echo "<textarea class='mybooktable_shortcode_field' id='mybooktable_{$shortcode}_field_$setting' name='$setting' rows='5' cols='40'></textarea>";
										break;
									case 'text':
										if ( ! empty($params['title']) ) {
											echo "<label for='mybooktable_{$shortcode}_field_$setting'>$params[title]";
										...
										if ( ! empty($params['description']) ) echo '<div class="description">' . $params['description'] . '</div>';
										echo "<input type='text' class='mybooktable_shortcode_field' id='mybooktable_{$shortcode}_field_$setting' name='$setting'>";
										break;
									case 'number':
										if ( ! empty($params['title']) ) {
											echo "<label for='mybooktable_{$shortcode}_field_$setting'>$params[title]";
									...
									default:
										if ( ! empty($params['title']) ) {
											echo "<label for='mybooktable_shortcode_field_$setting'>$params[title]";
											if ( ! empty($params['default']) ) echo " <em>(default: $params[default])</em>";
											echo "</label><br>";
										}
										if ( ! empty($params['description']) ) echo '<div class="description">' . $params['description'] . '</div>';
...
?>
	...
	<pre>[mybooktable tag="reccomended"]</pre>
